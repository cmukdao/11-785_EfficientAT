...
def load_net_from_dict(net, state_dict):
    ...
    try:
        ...
        if set(net.state_dict().keys()) == set(state_dict.keys()):
            ...
            # Filter out PBP-specific tracking buffers (dendrite_values.*)
            # that are not present in models initialized without PBP
            filtered = {k: v for k, v in state_dict.items()
                        if 'dendrite_values' not in k}
            if set(net.state_dict().keys()) == set(filtered.keys()):
                print("Loading state_dict after filtering PBP tracking buffers")
                net.load_state_dict(filtered)
            else:
